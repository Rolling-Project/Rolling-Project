module.exports = {
  env: { // 프로젝트 환경 지정(브라우저 및 ECMAScript 2021)
    browser: true, 
    es2021: true,
  },
  extends: 'airbnb', // Aribnb의 JavaScript 스타일 가이드 기반
  overrides: [ // 특정 환경 또는 파일에 대한 추가적인 설정 정의
    {
      env: { // 해당 설정이 적용될 환경 정의
        node: true,
      },
      files: [ // 설정이 적용될 파일 정의
        '.eslintrc.{js,cjs}',
      ],
      parserOptions: { // ES Module 형식의 스크립트 파일 사용(import/export)
        sourceType: 'module',
      },
    },
  ],
  parserOptions: { // 코드를 해석하는데 필요한 parse 옵션 정의(가장 최신 ECMAScript 사용, 모듈)
    ecmaVersion: 'latest',
    sourceType: 'module',
  },
  rules: { // ESLint 규칙을 추가하거나 재정의
